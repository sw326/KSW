...
3) 단계 ( step )
	1) 솔루션(Solution) 제작 순서
		1) 현실세곙,; '업무(프로세스)분석' / '요구사항'
		2) '개념적' 모델링 (PPT, txt, ... )
		3) '논리적' 모델링(diagram with tool)
		4) '물리적' 모델링(diagram with tool)
		5) SQL생성 ( DDL, DML, DQL )
		6) 구현( 디자인 + 프로그램 )
		7) 테스팅
		8) 배포 / 서비스 ( 유지보수 )
	...
	2) DB 모델링 순서
		...
		3) '논리적' 모델링
		   개념적 모델링 단계에서 정의된 ERD(Entity Relationship Diagram) 매핑룰을 적용해서 스키마를 생성하고 '정규화'까지 포함하는단계
		   ( 스키마는 개체들의 관계 제약조건등의 명세를 포함한 '데이터 구조' )
		...
		5) SQL / PLSQL
			...
			2) drop.sql
			3) insert.sql
			4) select.sql ( '욕 사항'을 기준으로 작성 )
		6) 
	3) ERD의 관계 표기
		1) 표기
			1. 실선 : '식별관계( Identifying )'
			   부모 테이블의 PK가 자식 테이블의 'FK / PK컬럼'이 되는 경우
			2. 점선 : '비식별관계( Non-Identifying )'
			   부모 테이블의 PK가 자식 테이블의 'FK / 일반 컬럼'이 되는 경우
			3. O : Optional - 선택 ( 0개가 될 수 있다. )
			4. | : Mandatory - 필수 ( 1가 있어야 한다. )
